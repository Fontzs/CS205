...
public:
    CCube() : CStereoShape() {}
    CCube(double length, double width, double height) : CStereoShape()
    {
        this->length = length;
        this->width = width;
        this->height = height;
    }
    CCube(const CCube &other) : CStereoShape(other)
    {
    ...
    double getArea() override
    {
        return 2 * (this->length * this->width +
                    this->length * this->height + this->width * this->height);
    }
...
public:
    ...
    CSphere(double radius) : CStereoShape()
    {
        this->radius = radius;
    }
    CSphere(const CSphere &other) : CStereoShape(other)
    {
    ...
    double getVolume() override
    {
        return 4.0 / 3 * PI * pow(this->radius, 3);
    }
    void show() override
    {
